...
const schemas ={
    ...
    user :Joi.object().keys({
          ...
          password:Joi.string().trim().required().pattern(new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})")),
...
//.pattern(new RegExp("^(?=.*[.])(?=.*[a-z])")).required().min(3).max(50).regex(/(?=.*[0-9])/, { invert: true }).regex(/(?=.*["  "])/, { invert: true })