...
exports.DATA = (async (req, res) => {
  ...
  try {
        ...
        if (line[i]?.includes('Connected')) {
          ...
          for (let j = i; j <= 1000; j++) {
            ...
            if (line[j]?.slice(line[j].indexOf('[') + 1, line[j].indexOf(']')) == mainId) {
              ...
              // console.log("second loop --------------",line[j]?.slice(line[j].indexOf('[') + 1, line[j].indexOf(']')))
              ...
              if (line[j].includes('MAIL FROM:')) {
                ...
                const obj =await connection2.execute(`SELECT *, CONCAT(U_Alias,'@',U_Domain) AS name FROM user where  CONCAT(U_Alias,'@',U_Domain) ="admin@icewarpdemo.com"`)
                
                let userObj = JSON.parse(JSON.stringify(obj[0]));
                
                name =userObj[0][0].U_Alias;
                sendLimit=userObj[0][0].U_MegabyteSendLimit;
                mailBoxQuota=userObj[0][0].U_MaxBoxSize;
                externalEmail=userObj[0][0].U_RemoteAddress;

                if(userObj[0][0].U_AccountDisabled == 0){
                  accountActivity="disabled"
                }else{
                  accountActivity= "enabled"
                }
          ...
          if (senderMailIds != null && receiverMailIds != null) {
            const [rows, fields] =
              await connection.execute(`INSERT INTO logs_data (name,main_id,sender_address,recipient_address,fom_ip,
                                       top_ip,email_size,status_val,date_time,sendLimit,mailBoxQuota,externalEmail,jeo_location,accountActivity,error_notification,user_creation_date) VALUES
                                       (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [(name || null),
              (mainId || null),
              (senderMailIds || null),
    ...
    // rsstream.on("end",()=>{
    //   res.json({message:"there will be not more"})
    // })
  } catch (error) {
    console.log(error)
  }
